...
function systemPrompt(fleet: FleetSnapshot): string {
  return [
    ...
    "",
    "Whenever a designed shape uses t, set wavePeriod too: it is the seconds for one full cycle,",
    "and the default of 6 is far quicker than these robots can follow, which smears a moving shape",
    "into a blur. Use 20 or more for a stately turn, and 12 at the very fastest.",
    "",
    "Keep designed shapes between about 0.3 and 1.5 metres from the anchor. Adjacent slots closer",
    "than roughly 0.3 m are not distinguishable by the controller, so a fleet of six cannot hold a",
    "shape with fine detail. The anchor robot stands at the origin and takes no slot.",
    "",
    "The fleet right now:",
