...
class Robot:
    def __init__(self, robot_id: int, initial_position: Position, world_size: float):
        self.id = robot_id
        # Convert integer ID to 6-character hex string (uppercase) for MQTT topics
        self.hex_id = f"{robot_id:06X}"
    ...
    def process_command(self, command):
        """Process movement commands according to the current protocol"""
        try:
            if command == "Stop":
                self.left_motor_power = 0.0
                self.right_motor_power = 0.0
            elif isinstance(command, dict) and "Move" in command:
                move_data = command["Move"]
                # Right left are mirrored.
                self.left_motor_power = max(-1.0, min(1.0, float(move_data.get('right', 0.0))))
                self.right_motor_power = max(-1.0, min(1.0, float(move_data.get('left', 0.0))))
            elif isinstance(command, dict):
                # Fallback to the old format if received as {"left": ..., "right": ...}
                self.left_motor_power = max(-1.0, min(1.0, float(command.get('right', 0.0))))
                self.right_motor_power = max(-1.0, min(1.0, float(command.get('left', 0.0))))
            else:
                # String fallbacks
                if command == 'l':
            ...
            print(f"Invalid command for robot {self.hex_id}: {command}, error: {e}")
